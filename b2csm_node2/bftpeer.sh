...
docker create --rm -it \
--network=bridge \
--name 1.peer.ent1.b2csm \
--privileged \
-p 7051:7051 \
-p 7053:7053 \
-v $(pwd)/peer_material/:/bft-config/ \
-v $(pwd)/crypto-config/peerOrganizations/ent1.b2csm/peers/1.peer.ent1.b2csm/msp:/bft-config/msp \
...
docker network connect b2csmnetwork 1.peer.ent1.b2csm
docker start -a 1.peer.ent1.b2csm
